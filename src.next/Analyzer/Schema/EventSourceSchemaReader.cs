...
namespace ChilliCream.Tracing.Schema
{
    ...
    /// </summary>
    internal static class EventSourceSchemaReader
    {
        ...
        public static EventSourceSchema GetSchema(EventSource eventSource)
        {
            if (eventSource == null)
            {
        ...
        internal static EventSourceSchema GetSchema(string manifestXml)
        {
            ...
            DeserializeEventSchemas(provider, eventSchemas, eventIdCounts);

            ImmutableDictionary<int, EventSchema> events = ImmutableDictionary<int, EventSchema>.Empty;
            Dictionary<int, List<EventSchema>> duplicateEvents = new Dictionary<int, List<EventSchema>>();

            foreach (EventSchema eventSchema in eventSchemas)
            {
                if (eventIdCounts[eventSchema.Id] > 1)
                {
                    List<EventSchema> errorEvents;
                    if (!duplicateEvents.TryGetValue(eventSchema.Id, out errorEvents))
                    {
                        errorEvents = new List<EventSchema>();
                        duplicateEvents[eventSchema.Id] = errorEvents;
                    }
                    errorEvents.Add(eventSchema);
                }
                else
                {
                    events = events.Add(eventSchema.Id, eventSchema);
                }
            }

            IReadOnlyCollection<EventSourceSchemaError> errors = CreateErrors(duplicateEvents);
            EventSourceSchema eventSourceSchema = new EventSourceSchema(providerGuid, providerName, events, errors);
            SetEventSourceProperty(eventSourceSchema, eventSchemas);
            return eventSourceSchema;
        }

        private static void SetEventSourceProperty(EventSourceSchema eventSourceSchema, IEnumerable<EventSchema> eventSchemas)
        {
            foreach (EventSchema eventSchema in eventSchemas)
            {
                eventSchema.EventSource = eventSourceSchema;
            }
        }

        private static IReadOnlyCollection<EventSourceSchemaError> CreateErrors(Dictionary<int, List<EventSchema>> duplicateEvents)
        {
            ImmutableArray<EventSourceSchemaError> errors = ImmutableArray<EventSourceSchemaError>.Empty;
            foreach (List<EventSchema> duplicateEventsList in duplicateEvents.Values)
            {
                errors = errors.Add(new EventSourceSchemaError(
                    EventSourceSchemaErrorCodes.DuplicateEventIds, duplicateEventsList));
            }
            return errors;
        }
        ...
        private static void TrackDuplicateEventIds(Dictionary<int, int> eventIdCounts, int eventId)
        {
            int count;
            if (!eventIdCounts.TryGetValue(eventId, out count))
            {
                count = 0;
            }
            eventIdCounts[eventId] = ++count;
        }

        private static EventSchema CreateEventSchema(int eventId, XElement @event, XElement provider)
        {
