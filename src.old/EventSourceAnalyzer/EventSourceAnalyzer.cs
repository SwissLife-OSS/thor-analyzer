...
using Microsoft.Practices.EnterpriseLibrary.SemanticLogging.Utility.Properties;
    ...
    public sealed class EventSourceAnalyzer
    {
        ...
        private class ProbeEventListener : EventListener
        {
            ...
            private void CheckPayload()
            {
                ...
                {
                    string errorMessage = string.Format(CultureInfo.CurrentCulture, Properties.Resources.EventSourceAnalyzerDifferentParameterCount, eventMethod.Name);

                    // check if the payload contains an error message;
                    if (this.EventData.Payload.Count > 0)
                    {
                        for (int i = 0; i < this.EventData.Payload.Count; i++)
                        {
                            string payload = this.EventData.Payload[i] as string;
                            if (!string.IsNullOrWhiteSpace(payload) && payload.StartsWith("error:", StringComparison.OrdinalIgnoreCase))
                            {
                                errorMessage += "\r\n" + payload;
                            }
                        }
                    ...
                    this.Error = new EventSourceAnalyzerException(errorMessage)
                    {
                        Event = this.EventData
                    };
                    return;
